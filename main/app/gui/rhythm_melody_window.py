...
and a hand position, hear the melody, and write the .mid/.json/.csv/.txt
stimulus files for the rhythm experiment.
...
The only shared thing it uses is ``note_audio.NoteAudioPlayer`` for the
Play button - the same GUI-free tone synthesiser the piano and quiz windows
use - which claims the audio output device and writes nothing.
...
target finger (L1-L5 / R1-R5) that never changes between repetitions. See
...
class RhythmMelodyWindow(QMainWindow):
    def __init__(self, cfg: Optional[Config] = None):
        ...
        self._sequence = None
        self._player: Optional[NoteAudioPlayer] = None
        self._schedule: List[Tuple[int, str, int, int]] = []
        self._cursor = 0
        ...
        body.addWidget(self._build_output(), 1)
    ...
    def _build_controls(self) -> QWidget:
        ...
        self.count_spin.setToolTip("Generates seed, seed+1, ... one file set each.")
        form.addLayout(_row("How many", self.count_spin))
        ...
        actions = QGroupBox("Preview && export")
        ...
        acts.addLayout(play_row)

        self.timbre_combo = QComboBox()
        for name in TIMBRES:
            self.timbre_combo.addItem(TIMBRES[name].name, name)
        index = self.timbre_combo.findData(DEFAULT_TIMBRE)
        if index >= 0:
            self.timbre_combo.setCurrentIndex(index)
        acts.addLayout(_row("Preview timbre", self.timbre_combo))
    ...
    def _build_output(self) -> QWidget:
        ...
        self.status = QLabel("Press Preview to generate a melody.")
    ...
    def _refresh_gate_hint(self) -> None:
        ...
        held = [f"{beats}-beat note sounds {sounding_beats(float(beats), timing):g}"
                for beats in (1, 2, 3)]
        self.gate_hint.setText("; ".join(held) + " beat(s).")

    def _refresh_layout_hint(self) -> None:
        layout = LAYOUTS[self.layout_combo.currentData()]
        keys = "  ".join(f"{slot.label}={_note_name(slot.midi)}" for slot in layout.slots)
    ...
    def _sync_keys_to_layout(self) -> None:
        ...
        if current >= 0 and KEYS[self.key_combo.itemData(current)].tonic_pc not in available:
    ...
    def _preview(self) -> None:
        ...
        self._show(sequence)
        self.status.setText(
            f"{self.status.text()}  -  nothing written; press "
            f"\"Generate && Save files\" to write the file set."
        )

    def _generate_and_save(self) -> None:
        self._stop_playback()
        first_seed = self.seed_spin.value()
        written: List[Path] = []
        last = None
        for offset in range(self.count_spin.value()):
            sequence = self._generate(first_seed + offset)
            if sequence is None:
                return
            try:
                paths = export_sequence(sequence, self._out_dir)
            except OSError as error:
                QMessageBox.warning(self, "Couldn't write the files", str(error))
                return
            written.append(paths["midi"])
            last = sequence
        if last is None:
            return
        self._show(last)
        ...
        self.status.setText(
            f"{self.status.text()}  -  wrote {len(written)} x 4 files to "
            f"{self._out_dir} ({names})"
        )
    ...
    def _show(self, sequence) -> None:
        ...
        self.status.setText(
            f"seed {sequence.seed}  -  {len(sequence.notes)} notes, "
    ...
    def _play(self) -> None:
        """Audition the melody through the shared tone synthesiser.

        Uses the note-on/note-off seconds from the generated sequence, so
        what you hear is exactly what the .mid file contains, gate times
        included.
        """
        ...
        if self._player is None:
            try:
                self._player = NoteAudioPlayer(timbre=self.timbre_combo.currentData())
            except Exception as error:  # no audio device, busy device, ...
                self.status.setText(f"No audio output available: {error}")
                return
        self._player.set_timbre(self.timbre_combo.currentData())

        events: List[Tuple[int, str, int, int]] = []
        for note in self._sequence.notes:
            events.append((int(note.note_on_time_sec * 1000), "on", note.midi_note,
                           note.event_index))
            events.append((int(note.note_off_time_sec * 1000), "off", note.midi_note,
                           note.event_index))
        events.sort(key=lambda item: (item[0], item[1] == "on"))
    ...
    def _tick(self) -> None:
        ...
        while self._cursor < len(self._schedule) and self._schedule[self._cursor][0] <= now:
            _when, action, midi, index = self._schedule[self._cursor]
            if self._player is not None:
                if action == "on":
                    self._player.play_key(midi)
                    note = self._sequence.notes[index]
                    self.status.setText(
                        f"playing {index + 1}/{len(self._sequence.notes)}  "
                        f"{note.note_name}  {note.finger}  "
                        f"({note.duration_beats:g} beat)"
                    )
                else:
                    self._player.stop_key(midi)
            self._cursor += 1
        if self._cursor >= len(self._schedule):
            self._stop_playback()
            self._show(self._sequence)
    ...
    def _stop_playback(self) -> None:
        ...
        self.stop_btn.setEnabled(False)
        self.play_btn.setEnabled(self._sequence is not None)
