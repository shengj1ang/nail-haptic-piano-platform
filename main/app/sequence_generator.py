...
main user study described in the final report's Methods chapter
(§"Controlled Stimulus Sequence Generator") and detailed in
the final report's implementation appendix.
...
the final report's Methods chapter - groups of measurable features, never a weighted scalar score
(component definitions: the final report's implementation appendix §"Difficulty Components"):
...
constrained component falls inside that level's ranges (the final report's Methods chapter Table
...
clique search, mirroring the final report's implementation appendix Table "Pairwise matching
...
# the final report's Methods chapter, §"Controlled Stimulus Sequence Generator": every formal
...
# user-configurable in the GUI (the final report's stimulus-register appendix records
# the locked pool as "3 difficulty levels x 9 sequences = 27").
DEFAULT_FAMILY_COUNT = 9
MIN_FAMILY_COUNT, MAX_FAMILY_COUNT = 1, 50

# Level-specific hand-region overlap ratio r (the final report's implementation appendix
# §"Hand Regions and
# Sequence Construction"): R_L(r) = [k_min, k_mid + rS/2],
# R_R(r) = [k_mid - rS/2, k_max].
LEVEL_OVERLAP_RATIO = {"alpha": 0.0, "beta": 0.15, "gamma": 0.30}

# the final report's Methods chapter Table "Level-defining constraints"; difficulty
...
# Structural rejection rules (the final report's implementation appendix §"Candidate
# Construction and Rejection Rules", first paragraph).
MAX_SINGLE_HAND_SHARE = 0.60
MAX_CONSECUTIVE_SAME_FINGER = 3

# Gamma-level cross-region ergonomic checks (the final report's implementation appendix
...
# the final report's implementation appendix Table "Pairwise matching tolerances for sequences
...
# (the final report's implementation appendix §"Difficulty Components"): 0 / (0,2] / (2,5] / >5.
...
def valid_notes_for_profile(keyboard_profile_name: str, profile_data_dir: Path = PROFILE_DATA_DIR) -> List[int]:
    ...
    note range (the final report's implementation appendix §"Hand Regions and Cross-Region
...
def white_notes_for_profile(keyboard_profile_name: str, profile_data_dir: Path = PROFILE_DATA_DIR) -> List[int]:
    ...
    V_white (the final report's Methods chapter §"Controlled Stimulus Sequence Generator":
...
class ComponentSpec:
    ...
    (the final report's implementation appendix §"Family Matching Tolerances")."""
...
def hand_regions(k_min: float, k_max: float, level: str) -> Dict[str, Tuple[float, float]]:
    ...
    the level-specific overlap ratio r (the final report's implementation appendix §"Hand Regions
...
def _transition_classes(actions: Sequence) -> List[Tuple[int, int, int]]:
    ...
    hand-switch transition (the final report's implementation appendix §"Difficulty Components")."""
    classes = []
...
def predictability(actions: Sequence) -> float:
    ...
    a_{t+1} = (h_{t+1}, f_{t+1}, m_{t+1}) given a_t (the final report's implementation appendix "Difficulty
...
def compute_stats(actions: Sequence, k_min: int, k_max: int) -> SequenceStats:
    ...
    agree (the final report's implementation appendix, last paragraph of "Family Matching
...
def structural_violations(actions: Sequence) -> List[str]:
    """The the final report's implementation appendix structural rejection rules, applicable to any
...
def _sample_hand_labels(level: str, rng: random.Random, max_attempts: int = 400) -> Optional[List[str]]:
    ...
    same-hand transition) in advance (the final report's implementation appendix "Hand Regions and
...
def _build_one_sequence(
    ...
    None (restart) when no legal continuation exists (the final report's implementation appendix "Hand
...
def generate_matched_family(
    ...
    matched within FAMILY_TOLERANCES (the final report's implementation appendix Table "Pairwise matching
...
def evaluate_song(
    ...
    numbers are directly comparable either way (the final report's implementation appendix: "A
